...
function renderMonthly() {
    ...
    if ($("monthlyList")) {
        $("monthlyList").innerHTML = list.length ? list.map(p => {
            ...
            return `<div class="card"><div class="check-card"><input type="checkbox" ${a.checked ? "checked" : ""} data-action="toggle-assignment" data-project-id="${p.id}" /><div><strong>${p.icon} ${esc(p.name)}</strong><div class="muted">Trésorerie : ${Currency.format(ProjectService.getKPIs(p).treasury, p.currency)}</div></div><div class="field"><input type="number" value="${a.amount || ""}" data-action="set-assignment-amount" data-project-id="${p.id}" ${a.checked ? 'disabled' : ''}/></div></div></div>`;
...
function toggleAssignment(projectId, checked) {
    ...
    if (checked) {
        ...
        if (!amount || amount <= 0) {
            alert("Saisissez d’abord un montant valide.");
            renderMonthly();
            return;
        }
